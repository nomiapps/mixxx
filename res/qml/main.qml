...
ApplicationWindow {
    ...
    // cascade position, place the window deterministically: centred on the
    // screen's available area, then clamped so the title bar is always
    // reachable. Runs deferred -- at Component.onCompleted the window manager
    // hasn't assigned the real screen/position yet.
    function clampOntoScreen() {
        ...
        if (!s) {
            console.warn("main.qml: clampOntoScreen: no screen available");
            return ;
        }
