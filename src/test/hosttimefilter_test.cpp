...
TEST_F(HostTimeFilterTest, LongRunningClocksStayAccurate) {
    HostTimeFilter filter(512);
    constexpr double kBufferSeconds = 512.0 / 48000.0;
    for (int i = 0; i < 100000; ++i) {
        const double auxiliaryTime = 72000.0 + i * kBufferSeconds;
        const auto hostTime = std::chrono::microseconds(
                1000000000000LL + std::llround(i * kBufferSeconds * 1000000.0));
        filter.insertTimePoint(auxiliaryTime, hostTime);
        if (i >= 512) {
            EXPECT_NEAR(filter.calcHostTime(auxiliaryTime).count(), hostTime.count(), 2);
        }
    }
}

TEST_F(HostTimeFilterTest, ZeroCapacityStillAcceptsClockSamples) {
    HostTimeFilter filter(0);
    filter.insertTimePoint(1.0, 1000us);
    filter.insertTimePoint(2.0, 2000us);
    EXPECT_EQ(filter.calcHostTime(2.0), 2000us);
}
